...
--
-- The default is a constant empty string rather than a timestamp expression because
-- SQLite's ALTER TABLE ADD COLUMN forbids a parenthesised or non-deterministic DEFAULT.
-- New rows get their value bound by the writer (internal/webhook). Rows that predate
-- this migration keep '', which sorts last under ORDER BY created_at DESC — correct,
-- since they are the oldest deliveries on the instance.
